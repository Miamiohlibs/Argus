...
6. Add ArchivesSpace details (if using ArchivesSpace).
   - Create an ArchivesSpace API user for Argus with "viewer" or "read-only" permissions to all repositories; include the credentials for that user as `ASPACE_API_USER`, `ASPACE_API_PASSWORD`.
...
8. Add optional local customizations to the `.env` file.
   ...
   - Edit `NEXT_PUBLIC_USER_AFFILIATIONS`. This is the list of affiliations available for a user or a project's patron, such as your institution's name and "Other". You can list as many affiliations as your institution needs (not just a single "us / other" split); order is preserved as given, so list your institution's own name(s) first.
