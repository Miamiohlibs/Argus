...
export async function duplicateProject(projectId: string) {
  ...
  try {
    ...
    const objToAdd: EntryActionData[] = bibEntries.map((entry) => {
      ...
      const bibData: BibDataDraft = {
        ...
        catalogIdType: entry.catalogIdType,
      ...
      const itemData: ItemDataDraft[] = entry.items.map((item, index) => ({
        clientKey: item.id ?? `item-${index}`,
